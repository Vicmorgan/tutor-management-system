      ...
      try {
        ...
        if (dayName && grouped[dayName]) {
          grouped[dayName].push({
            ...
            course: `Course ${session.course_id}`, // Mock
            tutor: `Tutor Name`, // Mock
            room: session.virtual_link ? 'Virtual' : 'Room A1',
            isVirtual: !!session.virtual_link
          });
        }
      } catch (e) {}
    });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-on-surface-variant mt-1">Your enrolled classes for this week.</p>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {days.map(d => (
          ...
          >
            {d}
            {(schedule[d] || []).length > 0 && <span className="ml-1.5 w-1.5 h-1.5 bg-current rounded-full inline-block align-middle opacity-60"></span>}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {todayClasses.length === 0 && (
          <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-16 text-center">
            <span className="material-symbols-outlined text-[48px] text-on-surface-variant opacity-40 block mb-2">event_available</span>
            <p className="text-on-surface-variant font-medium">No classes on {activeDay} — enjoy your free time!</p>
          </div>
        )}
        {todayClasses.map((cls, i) => (
          <div key={i} className="bg-surface-container-lowest rounded-2xl border border-outline-variant p-6 flex items-center gap-6">
            <div className="text-center w-28 shrink-0">
              <p className="font-extrabold text-primary">{cls.time.split(' – ')[0]}</p>
            ...
            <div className="flex-1">
              <h3 className="font-extrabold text-on-surface">{cls.course}</h3>
              <p className="text-sm text-on-surface-variant mt-0.5">{cls.tutor}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="material-symbols-outlined text-[14px] text-on-surface-variant">{cls.isVirtual ? 'videocam' : 'location_on'}</span>
